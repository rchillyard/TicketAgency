...
case class CreateTicketPool(ts: Set[Ticket], replyTo: ActorRef[Response]) extends Request
...
case class ProformaTransaction(quantity: Int, price: Int, replyTo: ActorRef[Transaction]) extends Response

case class TicketBlock(ts: Set[Ticket]) extends Transaction

case class SeatRequest(quantity: Int, price: Int, replyTo: ActorRef[Response]) extends Request

case class Seats(ts: Set[Ticket], replyTo: ActorRef[Response]) extends Request
